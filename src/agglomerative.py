...
for i in range(2, 100):
    N_CLUSTERS = i
    print("Clustering the data into {0} clusters...".format(N_CLUSTERS))
    # dev - shorten array. keep crashing the process

    clustering = AgglomerativeClustering(n_clusters=N_CLUSTERS)
    y_clustering = clustering.fit_predict(components)
    labels = clustering.labels_

    # cast predictions to a dataframe
    y_cluster_df = pd.DataFrame(y_clustering)
    y_cluster_df.columns = ["Prediction"]
    complete_df = pd.concat([principle_components, y_cluster_df], axis=1)
    print(complete_df.head())

    # dunn_df = pd.DataFrame()
    cluster_list = []
    for j in range(N_CLUSTERS):
        label = "cluster_" + str(j)
        cluster = complete_df.loc[complete_df.Prediction == j]
        cluster_list.append(cluster)
        # dunn_df[label] = cluster
    # dunn = base.dunn(cluster_list)
    # dunn = sp.posthoc_dunn(cluster_list)
    # cluster_list = [[1,2,3,5], [12,31,54, np.nan], [10,12,6,74,11]]
    dunn = sp.posthoc_dunn(cluster_list)
    print("Dunn: ")

    scores[N_CLUSTERS] = silhouette_score(X, labels, metric="euclidean")
...
# N_COMPONENTS = 2
# print("Decomposing the data into principle components")
...
# get a PCA helper
# start simple with two components
# TODO - get more components and select the first n based on some threshold?
pca = PCA(n_components=N_COMPONENTS)
...
# scores["PCA3"] = silhouette_score(X, labels, metric="euclidean")
...
# scores["no PCA"] = silhouette_score(X, labels, metric="euclidean")
